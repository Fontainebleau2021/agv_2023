...
public:
    ...
    //pcl::PointCloud<PointTypePose>::Ptr keyframeMap(new pcl::PointCloud<PointTypePose>);
    ...
    void updateInitialGuess_liosam()
    {
...
#pragma omp parallel for num_threads(numberOfCores)
	...
	//cloudScanForInitialize->clear();
        ...
        float icp_min_score = 0.3;
        int icp_min_id = 0;
        icp.setInputSource(laserCloudIn);
        icp.setInputTarget(cloudGlobalMapDS);
        pcl::PointCloud<PointTypePose>::Ptr keyframeMap(new pcl::PointCloud<PointTypePose>);
        pcl::io::loadPCDFile(std::getenv("HOME") + savePCDDirectory + "transformations.pcd", *keyframeMap);
        for (int i = 0; i < keyframeMap->points.size();i++)
        {
            //global transformation
            transformInTheWorld[0] = keyframeMap->points[i].roll;
            transformInTheWorld[1] = keyframeMap->points[i].pitch;
            transformInTheWorld[2] = keyframeMap->points[i].yaw;
            transformInTheWorld[3] = keyframeMap->points[i].x;
            transformInTheWorld[4] = keyframeMap->points[i].y;
            transformInTheWorld[5] = keyframeMap->points[i].z;
            ...
            if (icp.getFitnessScore()<icp_min_score)
            {

                icp_min_score = icp.getFitnessScore();
                icp_min_id = i; 
                break;
            }
        }
        ...
        transformInTheWorld[0] = keyframeMap->points[icp_min_id].roll;
        transformInTheWorld[1] = keyframeMap->points[icp_min_id].pitch;
        transformInTheWorld[2] = keyframeMap->points[icp_min_id].yaw;
        transformInTheWorld[3] = keyframeMap->points[icp_min_id].x;
        transformInTheWorld[4] = keyframeMap->points[icp_min_id].y;
        transformInTheWorld[5] = keyframeMap->points[icp_min_id].z;
        ...
        laserOdometryROS.header.frame_id = "odom";
        laserOdometryROS.child_frame_id = "odom_mapping";
        laserOdometryROS.pose.pose.position.x = tranformOdomToWorld[3];
        laserOdometryROS.pose.pose.position.y = tranformOdomToWorld[4];
        laserOdometryROS.pose.pose.position.z = tranformOdomToWorld[5];
        laserOdometryROS.pose.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(tranformOdomToWorld[0], tranformOdomToWorld[1], tranformOdomToWorld[2]);
        laserOdometryROS.pose.covariance[0] = double(imuPreintegrationResetId);
        pubOdomAftMappedROS.publish(laserOdometryROS);
        */
        //publishOdometry();
        if (icp.hasConverged() == false || icp.getFitnessScore() > historyKeyframeFitnessScore)
        {
            ...
            pose_odomTo_map.pose.position.x = deltax; pose_odomTo_map.pose.position.y = deltay; pose_odomTo_map.pose.position.z = 0;//deltaz;
            ...
            tf::Transform t_odom_to_map = tf::Transform(tf::createQuaternionFromRPY(tranformOdomToWorld[0], tranformOdomToWorld[1], tranformOdomToWorld[2]),
                                                        tf::Vector3(tranformOdomToWorld[3], tranformOdomToWorld[4], tranformOdomToWorld[5]));
            tf::StampedTransform trans_odom_to_map = tf::StampedTransform(t_odom_to_map, timeLaserInfoStamp, "odom", "map");
