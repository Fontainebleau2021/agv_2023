...
from geometry_msgs.msg import PolygonStamped, Point32, Quaternion, PoseAr
from nav_msgs.msg import p
...
def feedback_callback(data):
  ...
  got_data = True
  arr = np.zeros([4, len(data.poses)], dtype='double'); # x, y, theta, v, omega, t
  # copy trajectory
  trajectories = []
  for index, pose in enumerate(data.poses):
    arr[0,index] = pose.position.x
    arr[1,index] = pose.position.y
...
#   trajectories.append({'raw': trajectory, 'mat': arr})
  ...
  mat = {'selected_trajectory_idx': data.selected_trajectory_idx, 'trajectories': trajectories}

  timestr = time.strftime("%Y%m%d_%H%M%S")
  filename = '/home/agv/' + 'teb_data_' + timestr + '.mat'
...
def feedback_exporter():
  ...
  topic_name = "/navi/teb_local_planner/teb_feedback" # define feedback topic here!
